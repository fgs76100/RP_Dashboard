...
AnalyzerTable.prototype.hide_description = function (id, hide) {
  let $element = $(id);
  if (hide) {
    $element.find('td:nth-child(n+4):nth-child(-n+5)').hide();
    $element.find('th:nth-child(n+4):nth-child(-n+5)').hide();
  } else {
    $element.find('td:nth-child(n+4):nth-child(-n+5)').show();
    $element.find('th:nth-child(n+4):nth-child(-n+5)').show();
  };
};
...
Card.prototype.Search = function () {
  ...
  $(`#addr_input${id}`).keypress( function (e) {
    if (e.which == 13) {
